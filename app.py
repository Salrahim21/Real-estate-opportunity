...
class RealEstateDashboard:
    ...
    def clean_geodata(self, gdf):
        """Comprehensive GeoDataFrame cleaning"""
    ...
    def load_data(self):
        """Load all datasets with proper timestamp handling"""
        try:
            # Load with comprehensive cleaning
            ...
            # Enhanced flood zone processing
            ...
            # Handle town names
            name_cols = [c for c in ['name', 'town_name', 'NAME', 'TOWN_NAME'] 
                        if c in self.towns.columns]
    ...
    def process_data(self):
        """Process opportunity scores with robust handling"""
    ...
    def create_widgets(self):
        ...
        # Flood zone styling controls
        self.flood_color = pn.widgets.ColorPicker(
            name='Flood Zone Color', value='#1E90FF')  # Dodger blue
        self.flood_opacity = pn.widgets.FloatSlider(
            name='Flood Zone Opacity', start=0.3, end=1.0, value=0.6, step=0.1)
        self.flood_edge_color = pn.widgets.ColorPicker(
            name='Flood Zone Edge Color', value='#0000FF')  # Blue
        self.flood_edge_width = pn.widgets.IntSlider(
            name='Flood Zone Edge Width', start=1, end=5, value=2, step=1)
        ...
        # Set up callbacks
        for widget in [self.opportunity_filter, self.heatmap_radius,
                     self.heatmap_intensity, self.flood_color,
                     self.flood_opacity, self.flood_edge_color,
                     self.flood_edge_width, self.show_roads,
                     self.show_towns, self.show_flood]:
            widget.param.watch(self.update_map, 'value')
    
    def create_map(self):
        """Create optimized map with patch-style flood zones"""
        m = folium.Map(
        ...
        # Add PATCH-STYLE FLOOD ZONES (key improvement)
        if self.show_flood.value and not self.flood.empty:
            folium.GeoJson(
                self.flood.__geo_interface__,
                style_function=lambda x: {
                    'fillColor': self.flood_color.value,
                    'color': self.flood_edge_color.value,
                    'fillOpacity': self.flood_opacity.value,
                    'weight': self.flood_edge_width.value,
                    'dashArray': '5, 3'  # Slightly dashed for better visibility
                },
                name='Flood Zones',
        ...
        if not filtered_grid.empty:
            HeatMap(
                [[point.y, point.x, score * self.heatmap_intensity.value] 
                 for point, score in zip(filtered_grid.geometry.centroid, 
                                       filtered_grid['opportunity_score'])],
    ...
    def create_layout(self):
        """Create dashboard layout with organized controls"""
        # Flood zone controls
        flood_controls = pn.Card(
            ...
            collapsed=False,
            styles={'background': '#f0f8ff'}  # Light blue background
        )
